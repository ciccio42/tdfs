...
namespace STMatch
{
  ...
  struct PatternPreprocessor
  {
    ...
    void get_partial_order()
    {
      ...
      for (int i = 0; i < pat.nnodes; ++i)
      {
        ...
        if (dep != -1)
        {
          ...
          for (int j = i - 1; j >= 0; --j)
          {
            if ((nbr_bits[i] & (1 << j)) > 0 && (nbr_bits[dep] & (1 << j)) == 0)
            {
              std::cout << j << " ";
    ...
    std::vector<std::pair<int, int>> GetConditions(bliss::Graph *bg)
    {
      ...
      auto eclass_it = std::find_if(eclasses.cbegin(), eclasses.cend(), [](auto &&e)
      ...
      while (eclass_it != eclasses.cend() && eclass_it->second.size() > 1)
      {
        ...
        for (auto &&perm : aut)
        {
          int min = *std::min_element(std::next(eclass.cbegin()), eclass.cend(), [perm](int n, int m)
                                      { return perm[n] < perm[m]; });
          result.emplace_back(n0, min);
        }
        aut.erase(std::remove_if(aut.begin(), aut.end(), [n0](auto &&perm)
                                 { return perm[n0] != n0; }),
                  aut.end());

        eclasses = GetAEquivalenceClasses(aut);
        eclass_it = std::find_if(eclasses.cbegin(), eclasses.cend(), [](auto &&e)
                                 { return e.second.size() > 1; });
      }
